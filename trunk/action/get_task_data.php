...
/* 获取关注任务的图标url */
$sql = "select task_id, task_info from task where task_type='follow' and (task_icon_url IS NULL OR task_screen_name IS NULL OR task_location IS NULL) limit 2";
$sql_res =  $dbo->getRs($sql);
foreach($sql_res as $user) {
    $user_info = $c->show_user_by_id($user['task_info']);
...
$sql = "select task_id, task_info from task where task_type='forward' and (task_icon_url IS NULL OR task_thumbnail_pic_url IS NULL OR task_bmiddle_pic_url IS NULL OR task_text IS NULL) limit 50";
$sql_res = $dbo->getRs($sql);
foreach($sql_res as $user) {
    $weibo = $c->show_status($user['task_info']);
